...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div id=lightBox class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><div class=lightbox-nav><button class=close aria-hidden=true ng-click=$dismiss()>×</button><div class=btn-group><a class=\"btn btn-xs btn-default\" ng-if=\"Lightbox.images.length > 1\" ng-click=Lightbox.prevImage()>‹ Previous</a> <a ng-href={{Lightbox.imageUrl}} target=_blank class=\"btn btn-xs btn-primary\" title=\"Open in new tab\" ng-if=\"Lightbox.type!='extension'\">Open in new tab</a> <a ng-href={{Lightbox.filename}} target=_blank class=\"btn btn-xs btn-primary\" title=\"Open in new tab\" ng-if=\"Lightbox.type=='extension'\">Open in new tab</a> <a class=\"btn btn-xs btn-default\" ng-if=\"Lightbox.images.length > 1\" ng-click=Lightbox.nextImage()>Next ›</a></div></div><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.imageCaption}}</span></div><img ng-if=!Lightbox.isVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} class={{Lightbox.thisclass}}><div ng-if=Lightbox.isVideo(Lightbox.image) class=\"embed-responsive embed-responsive-16by9\"><video ng-if=!Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} controls autoplay></video><embed-video ng-if=Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} ng-href={{Lightbox.imageUrl}} iframe-id=lightbox-video class=embed-responsive-item><a ng-href={{Lightbox.imageUrl}}>Watch video</a></embed-video></div></div></div>"
  );
...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.getClass = function (image) {
    return image.thisclass;
  };

  this.getType = function (image) {
    return image.type;
  };

  this.getFileName = function (image) {
  ...
  this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader',
    ...
    Lightbox.getClass = this.getClass;
    Lightbox.calculateImageDimensionLimits = this.calculateImageDimensionLimits;
    Lightbox.calculateModalDimensions = this.calculateModalDimensions;
    Lightbox.isVideo = this.isVideo;
    Lightbox.isSharedVideo = this.isSharedVideo;
    Lightbox.getType = this.getType;
    Lightbox.getFileName = this.getFileName;
    ...
    Lightbox.setImage = function (newIndex) {
      ...
      var success = function (properties) {
        ...
        Lightbox.thisclass = properties.thisclass || Lightbox.getClass(image);
        Lightbox.type = properties.type || Lightbox.getType(image);
        Lightbox.filename = properties.filename || Lightbox.getFileName(image);
